...
describe('ResultSummary', () => {
  ...
  it('正常にレンダリングされ、各指標が表示される', () => {
    ...
    expect(screen.getByText('5.56 %')).toBeInTheDocument(); // toFixed(2)で丸められる
  });

  it('値が0の場合も正しく表示される', () => {
    render(<ResultSummary totalPayment={0} initialIncome={0} cashFlow={0} yield={0} />);

    expect(screen.getByText('0 万円')).toBeInTheDocument(); // 総支払額
    expect(screen.getByText('0 万円')).toBeInTheDocument(); // 初年度収入
    expect(screen.getByText('0 万円')).toBeInTheDocument(); // 年間キャッシュフロー
